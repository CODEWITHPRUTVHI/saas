...
export default function SignupPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="absolute bottom-[-20%] right-[-10%] w-[600px] h-[600px] rounded-full opacity-15 pointer-events-none"
      ...
      <div className="w-full max-w-[480px] relative z-10">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                ...
                <input type="text" required autoComplete="name" value={name} onChange={(e) => setName(e.target.value)}
                  placeholder="Alex Johnson" className="w-full px-3.5 py-2.5 rounded-xl text-sm font-medium outline-none transition-all"
                  style={inputStyle} onFocus={onFocus} onBlur={onBlur} />
              </div>
              <div className="space-y-1.5">
                <label className="text-xs font-semibold uppercase tracking-wider" style={{ color: "#8697C4" }}>Brand / Company</label>
                <input type="text" value={brand} onChange={(e) => setBrand(e.target.value)} placeholder="Acme Corp"
            ...
            <div className="space-y-1.5">
              ...
              <input type="email" required autoComplete="email" value={email} onChange={(e) => setEmail(e.target.value)}
                placeholder="you@company.com" className="w-full px-4 py-3 rounded-xl text-sm font-medium outline-none transition-all"
                style={inputStyle} onFocus={onFocus} onBlur={onBlur} />
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <button type="button" onClick={() => setShowPass((v) => !v)}
                  className="absolute right-3.5 top-1/2 -translate-y-1/2 transition" style={{ color: "#8697C4" }}>
              ...
              {password.length > 0 && (
                <div className="space-y-1">
                  <div className="h-1.5 rounded-full overflow-hidden" style={{ background: "#ADBBDA" }}>
                    <div className="h-full rounded-full transition-all duration-500" style={{ width: strengthInfo.width, background: strengthInfo.color }} />
                  </div>
                  <p className="text-[11px] font-semibold" style={{ color: strengthInfo.color }}>{strengthInfo.label}</p>
                </div>
              )}
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                <input type={showPass ? "text" : "password"} required value={confirm} onChange={(e) => setConfirm(e.target.value)}
                  placeholder="Same as above" className="w-full px-4 py-3 pr-12 rounded-xl text-sm font-medium outline-none transition-all"
                  style={{ ...inputStyle, borderColor: confirm && password !== confirm ? "#ef4444" : "#ADBBDA" }}
            ...
            <div className="flex items-start gap-2.5">
              ...
              <span className="text-sm leading-relaxed" style={{ color: "#8697C4" }}>
                I agree to DROX's{" "}
                <a href="#" className="font-bold hover:underline" style={{ color: "#3D52A0" }}>Terms of Service</a>
                {" "}and{" "}
                <a href="#" className="font-bold hover:underline" style={{ color: "#3D52A0" }}>Privacy Policy</a>
              </span>
            </div>
        ...
        <p className="text-center text-xs mt-5 font-medium" style={{ color: "rgba(255,255,255,.50)" }}>
